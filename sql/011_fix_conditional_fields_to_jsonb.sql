...
-- Se usan variables temporales asignadas condicionalmente, accediendo a los campos de `NEW` a través de `to_jsonb(NEW)`
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'faltantes' THEN
        v_delivery_note := (to_jsonb(NEW)->>'delivery_note');
        v_olpn := (to_jsonb(NEW)->>'olpn');
        v_tipo_reporte := (to_jsonb(NEW)->>'tipo_reporte');
