-- CORRECCIÓN FINAL Y DEFINITIVA DE LA LÓGICA DE DESTINATARIOS
-- Se actualiza la función para que consulte ÚNICAMENTE la tabla `user_locals` para los roles de tienda,
-- ya que la columna `local_asignado` no existe en `public.profiles`.
...
BEGIN
    ...
    -- CORRECCIÓN FINAL: La consulta ahora solo usa `user_locals` para roles de tienda.
    ...
    WHERE 
        ...
        OR
        -- Condición 2: Roles de tienda que reciben notificaciones si el local del ticket está en su lista de locales asignados (tabla user_locals).
        (
